...
class WorkflowResponseConverter:
    ...
    def workflow_start_to_stream_response(
        ...
        *,
        ...
        # takin code: add created_by_role and created_by
        user_email = None
        if created_by_role == CreatorUserRole.ACCOUNT:
            stmt = select(Account).where(Account.id == created_by)
            account = session.scalar(stmt)
            if account:
                user_email = account.email
        elif created_by_role == CreatorUserRole.END_USER:
            stmt = select(EndUser).where(EndUser.id == created_by)
    ...
    def workflow_finish_to_stream_response(
        ...
        *,
        ...
        task_id: str,
        workflow_execution: WorkflowExecution,
        pricing_node_executions: dict[str, NodeExecution],
    ) -> WorkflowFinishStreamResponse:
        created_by = None
